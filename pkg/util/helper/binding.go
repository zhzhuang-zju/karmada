...
func NewDispenser(numReplicas int32, init []workv1alpha2.TargetCluster, lastResult []workv1alpha2.TargetCluster) *Dispenser {
	cp := make([]workv1alpha2.TargetCluster, len(init))
	copy(cp, init)
	return &Dispenser{NumReplicas: numReplicas, Result: cp, TieBreaker: tieBreakerByLastResult(lastResult)}
}
...
func tieBreakerByLastResult(lastResult []workv1alpha2.TargetCluster) func(a, b Party) bool {
	var lastResultMap = make(map[string]int32, len(lastResult))
	for _, cluster := range lastResult {
		lastResultMap[cluster.Name] = cluster.Replicas
	}
	return func(a, b Party) bool {
		// The party with fewer seats wins the tie.
		if a.Seats != b.Seats {
			return a.Seats < b.Seats
		}

		if lastResultMap[a.Name] != lastResultMap[b.Name] {
			return lastResultMap[a.Name] > lastResultMap[b.Name]
		}
...
func SpreadReplicasByTargetClusters(numReplicas int32, tcs, init []workv1alpha2.TargetCluster) []workv1alpha2.TargetCluster {
	...
	disp := NewDispenser(numReplicas, init, nil)
