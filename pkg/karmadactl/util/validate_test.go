...
func TestVerifyWhetherClustersExist(t *testing.T) {
	clusters := v1alpha1.ClusterList{Items: []v1alpha1.Cluster{
		{
	...
	tests := []struct {
		...
		},
		{
			...
			clusters: &v1alpha1.ClusterList{Items: make([]v1alpha1.Cluster, 0)},
			wantErr:  fmt.Errorf("clusters don't exist: member1,member2"),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if isExist := VerifyWhetherClustersExist(tt.input, tt.clusters); !isErrorEqual(tt.wantErr, isExist) {
				t.Errorf("VerifyWhetherClustersExist want: %v, actually: %v", tt.wantErr, isExist)
			}
		})
	}
}
