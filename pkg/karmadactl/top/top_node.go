...
func (o TopNodeOptions) RunTopNode(f util.Factory) error {
	...
	for _, cluster := range o.Clusters {
		go o.getNodeMetrics(&wg, &mux, f, cluster, selector, &allErrs)
	}
	wg.Wait()
...
func (o TopNodeOptions) getNodeMetrics(wg *sync.WaitGroup, mux *sync.Mutex, f util.Factory,
	cluster string, selector labels.Selector, allErrs *[]error) {
	defer wg.Done()

	nodeClient, metricsClient, err := GetCoreV1ClientAndMetricsClient(f, cluster, o.UseProtocolBuffers)
	if err != nil {
	...
	if len(metrics.Items) == 0 {
		*allErrs = append(*allErrs, fmt.Errorf("cluster(%s): metrics not available yet", cluster))
		return
	}

	nodes, err := getNodes(nodeClient, selector, o.ResourceName)
	if err != nil {
		*allErrs = append(*allErrs, fmt.Errorf("cluster(%s): %s", cluster, err))
		return
	}
...
func getNodes(nodeClient corev1client.NodesGetter, selector labels.Selector, resourceName string) ([]corev1.Node, error) {
	var nodes []corev1.Node
	if len(resourceName) > 0 {
		node, err := nodeClient.Nodes().Get(context.TODO(), resourceName, metav1.GetOptions{})
		if err != nil {
			return nil, err
		}
