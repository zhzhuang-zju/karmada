...
// WithGRPC sets the grpc config for scheduler
func WithGRPC(port int, certFile, keyFile, trustedCAFile string, insecureSkipVerify bool) Option {
	return func(o *schedulerOptions) {
		o.grpcConfig = &grpcutil.Config{
			ServerPort:         port,
			CertFile:           certFile,
			KeyFile:            keyFile,
...
func NewScheduler(dynamicClient dynamic.Interface, karmadaClient karmadaclientset.Interface, kubeClient kubernetes.Interface, opts ...Option) (*Scheduler, error) {
	...
	if options.enableSchedulerEstimator {
		...
		sched.grpcConfig = options.grpcConfig
...
func (s *Scheduler) reconcileEstimatorConnection(key util.QueueKey) error {
	...
	return estimatorclient.EstablishConnection(s.KubeClient, name, s.schedulerEstimatorCache, s.schedulerEstimatorServicePrefix, s.grpcConfig)
}
...
func (s *Scheduler) establishEstimatorConnections() {
	...
	for i := range clusterList.Items {
		...
		if err = estimatorclient.EstablishConnection(s.KubeClient, clusterList.Items[i].Name, s.schedulerEstimatorCache, s.schedulerEstimatorServicePrefix, s.grpcConfig); err != nil {
			klog.Error(err)
		}
	}
}
