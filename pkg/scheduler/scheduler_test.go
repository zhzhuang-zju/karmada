...
func TestCreateScheduler(t *testing.T) {
	...
	testcases := []struct {
		...
		{
			name: "scheduler with enableSchedulerEstimator disabled, WithGRPC enabled",
			opts: []Option{
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.enableSchedulerEstimator && tc.schedulerEstimatorPort != sche.grpcConfig.ServerPort {
				t.Errorf("unexpected schedulerEstimatorPort want %v, got %v", tc.schedulerEstimatorPort, sche.grpcConfig.ServerPort)
			}
		})
	}
}
